...
const createMedicine = async (req, res) => {
  ...
  try {
    ...
    if (existingMedicine) {
      return res
        .status(409)
        .json({ message: "Thuốc này đã tồn tại, vui lòng kiểm tra và update" });
    }
...
const getAllMedicine = async (req, res) => {
  ...
  try {
    ...
    if (isPatient) {
      return res
        .status(403)
        .json({ message: "Bệnh nhân không có quyền truy cập vào kho thuốc" });
    }
...
const updateMedicine = async (req, res) => {
  try {
    ...
    if (!(await checkAdmin(userId))) {
      return res
        .status(403)
        .json({ message: "Không phải admin thì không update được" });
    }
...
const deleteMedicine = async (req, res) => {
  try {
    ...
    if (!(await checkAdmin(userId))) {
      return res
        .status(404)
        .json({ message: "Không phải admin thì không xóa được" });
    }
  ...
  try {
    ...
    if (!(await checkAdmin(userId)) && !(await checkDoctor(userId))) {
      return res
        ...
        .json({
          message:
            "Không phải admin hoặc bác sĩ thì không xem chi tiết của cái thuốc này được",
        });
    }
